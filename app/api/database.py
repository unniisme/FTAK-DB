...
class FARMERdb(FTAKdb):
    ...
    def get_depots(self):
        query = f"SELECT i.depot_id, depot.name, depot.address_id FROM {self.farmer_info_view} as i JOIN depot ON i.depot_id = depot.depot_id"
        return self.dql_to_dictList(query)
    ...
    def get_products(self):
        query = f"SELECT p.product_id, p.name, p.description, p.rate, p.image_link FROM {self.farmer_info_view} as i JOIN product as p ON i.product_id = p.product_id"
        return self.dql_to_dictList(query)
